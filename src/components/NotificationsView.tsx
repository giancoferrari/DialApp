    ...
    <div style={{ maxWidth: 680, margin: '0 auto', padding: `${isMobile ? 28 : 48}px ${px}px ${isMobile ? 120 : 80}px` }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {friendReqs.map(f => (
            <div key={f.id} style={{ background: '#FAF6EA', border: '1px solid #E0D8C5', borderRadius: 18, padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          {matchInvites.map(m => {
            ...
            return (
              <div key={m.id} style={{ background: '#FAF6EA', border: '1px solid #E0D8C5', borderRadius: 18, overflow: 'hidden' }}>
                <div style={{ background: '#1F3A2A', padding: '12px 18px', display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ padding: '14px 18px' }}>
                  <div style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontSize: 18, fontWeight: 700, color: '#1F1D17', letterSpacing: '-0.02em', marginBottom: 4 }}>
                    {m.courseName}
                  </div>
                  <div style={{ fontSize: 13, color: '#6B6857', marginBottom: 4 }}>
                    {MODE_LABELS[m.gameMode] ?? m.gameMode} · {m.holes} holes
                    {m.wagerPerPlayer > 0 ? ` · $${m.wagerPerPlayer} wager` : ''}
                  </div>
                  <div style={{ fontSize: 12, color: '#B5AC95' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
                    <button
                      ...
                      disabled={busy === m.id}
                      style={{ flex: 1, background: '#F0EBDD', border: '1px solid #E0D8C5', borderRadius: 10, padding: '10px', fontSize: 13, color: '#6B6857', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}
                    >
                      Decline
                    </button>
                    <button
