...
- **Design tokens — `src/lib/tokens.ts` is the single source of truth.** Exports `font`, `space` (strict 4/8pt scale), `radius` (concentric scale), `color` (semantic names over the palette), `elevation` (3 levels), `z`, `motion`/`ease`, and `type` presets. New/edited code should reference these instead of magic numbers. (A full migration of every legacy inline literal onto the tokens is in progress — see the changelog at the bottom.)
- **Font:** **Helvetica, self-hosted.** The app uses **Nimbus Sans** — URW's open-licensed, metric-faithful Helvetica clone — shipped as `public/fonts/NimbusSans-{Regular,Bold}.woff2` and aliased via `@font-face` to the family name **`'Helvetica Neue'`** in `index.css`. This means genuine Helvetica letterforms render on **every** device (Windows/Android included), not just Apple — the system stack alone fell back to Arial off-Apple, which read as generic. The app-wide stack stays `'Helvetica Neue', Helvetica, Arial, sans-serif`, so the web font wins everywhere and the real system Helvetica/Arial remain as fallback if it fails. Only two weights are shipped (400, 700); CSS font-matching maps 500→Regular and 600→Bold. (woff2 generated from the URW OTFs with `wawoff2`, a one-time build step — not a runtime dep.) Numbers that sit in columns use `.tnum` / `numeric` (`font-variant-numeric: tabular-nums`). The old Bricolage/DM Sans pairing is gone.
- **Colors:** dark green `#1F3A2A` (primary), cream `#FAF6EA` / `#EDE8D4` (surfaces/bg), orange `#D9824D` (accent/CTA — use sparingly, ~one per screen), text `#1F1D17` (primary), `#4A4235`/`#6B5F4E`/`#8B8272` (secondary/muted/faint). Score colors: birdie green `#5C7A4D`/`#8BC47A`, bogey orange `#D9824D`, eagle gold `#C8A84B`, double+ red `#C0392B`. All available as semantic names in `color.*`.
...
- **Surfaces (`src/lib/surfaces.ts`):** the "subtract" rule in code — `card` (flat: solid `#FFFDF8` + 1px hairline border, **no blur, no shadow**), `cardRaised` (one hero per screen — whisper of `elevation.sm`), `inputSurface` (sunken sand), `sectionLabel`. **Backdrop-blur is being removed** from card surfaces app-wide (the loudest "AI" tell); it's kept only for the floating mobile bottom nav and modal/sheet scrims. Use these presets for any new surface instead of re-inventing frosted-glass cards.
...
_Last updated: 2026-06-03 — **Phases 6–7:** first-run **onboarding** flow (branded welcome → name/username/country/handicap/course/photo → finish, gated for new users) and **social feel** (inline feed comment preview, profile tabular numbers + rank halo). Earlier: Phases 1–5 — React Query, skeletons, toasts, double-tap like, gestures/haptics, design tokens (`lib/tokens.ts`), system **Helvetica** font, shared Avatar/EmptyState/format primitives, directional page transitions + spring modal entrances. 
