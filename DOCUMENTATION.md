...
## 5. Design system — **v2 (June 2026 redesign — COMPLETE, all screens)**

> The full visual redesign is **done across every screen.** Art direction = "Private Club": cool light-neutral content surfaces + immersive deep-green heroes on the identity/trophy screens. Full spec in repo-root `DESIGN.md`.

- **Design tokens — `src/lib/tokens.ts` is the single source of truth.** Exports `font`, `space` (4/8pt scale), `radius` (concentric: 10 controls → 12 buttons → 16 cards → 20 hero → 24 sheets), `color` (semantic names), `elevation` (flat/sm/md/lg), `z`, `motion`/`ease`, `type` presets, plus **`HERO_BG`** + **`onHero`** (text colors for the dark hero). Never hardcode hexes or decimal font sizes in new code.
- **Fonts:** **Space Grotesk** (`font.display`) for display only — scores, points numerals, page titles, section headers (athletic grotesk, -0.02em tracking). **Geist** (`font.body`) for ALL functional UI. **Bricolage Grotesque** (`font.wordmark`) for the multicolor logo only. All from Google Fonts in `index.html`. Tabular numerals on score/stat columns. **No uppercase eyebrow labels** — section labels are sentence case 13px/600. (A serif-display experiment, Fraunces, was rejected as dated.)
- **Logo:** the original multicolor `DialWordmark` is restored — Bricolage, D/l green, i/period orange, a sage; `onDark` variant for green heroes.
- **The "Private Club" hero** (`HERO_BG` + `CourseContour` + `onHero` tokens): immersive deep-green panel with faint topographic course linework, a `DialRing`-wrapped avatar and tier-colored progress. Full-bleed on mobile (with the porcelain content sheet docking over it via rounded top corners); a contained rounded block on desktop. On **Home**, mobile **TopNav takes `tone="dark"`** (set in `App.tsx`) to blend in. Used on Home, Profile, Matches (wallet), Leaderboard, Stats. `CourseContour.tsx` is the shared brand-landscape SVG.
...
### Redesign v2 (2026-06, COMPLETE — "Private Club")
...
_Last updated: 2026-06-11 — **Redesign v2 complete** across all screens (see above). Earlier: Phases 6–7 — first-run **onboarding** flow and **social feel**. Phases 1–5 — React Query, skeletons, toasts, double-tap like, gestures/haptics, design tokens, shared primitives, directional page transitions + spring modal entrances. 
