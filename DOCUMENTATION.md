...
- **Font:** the original pairing — **`Bricolage Grotesque`** (display/headings/large numbers) + **`DM Sans`** (body/UI), loaded from **Google Fonts** in `index.html`; `body` defaults to DM Sans in `index.css`. Exposed as `font.display` / `font.body` in `tokens.ts` (new components use those). Hierarchy via size/weight/tracking; numbers that sit in columns use `.tnum` / `numeric` (`font-variant-numeric: tabular-nums`). *(A self-hosted Helvetica experiment was tried and reverted — back to Bricolage + DM Sans, the look the app has always had.)*
