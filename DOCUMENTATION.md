...
- **Font:** **system Helvetica stack** — `'Helvetica Neue', Helvetica, Arial, sans-serif` for the entire app (single family; hierarchy via size/weight/tracking). Real Helvetica Neue on Apple, Arial on Windows, platform grotesque on Android. **No web fonts are downloaded** (the Google Fonts `<link>` was removed from `index.html`; `body` font set in `index.css`). The old Bricolage/DM Sans pairing is gone. Numbers that sit in columns use `.tnum` / `numeric` (`font-variant-numeric: tabular-nums`).
