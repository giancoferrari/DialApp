...
- **v4 conversion status:** all warm-cream screens + **Rounds/Scorecard** are on v4 (see §5). Remaining v2 hold-out: **`MatchesView.tsx`** (next in the redesign pass). The deep-pine `HERO_BG`/`onHero` tokens remain for the Messages thread header (intentional pine accent); `CourseContour.tsx` is now unused (slated for deletion in the stragglers prompt).
