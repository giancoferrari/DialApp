...
- **Mobile bottom nav (TopNav):** **floating frosted pill** (2026-06-13) — inset 14px from the screen edges, `borderRadius 28`, blur 20, soft drop shadow, floats `safe-area-bottom + 10px` above the bottom; icon + 12/600 label, active = ink label + a small **orange dot** below it. **Motion (2026-06-13, Emil pass):** the pill **rises in** on first mount (`.nav-pill-in`); on tab switch the active **dot springs in** (scale 0.2→1 with a subtle overshoot `cubic-bezier(0.34,1.56,0.64,1)`) and the active **icon lifts** 1px; press scales the tab to 0.92. Top bar on mobile is a transparent **absolute overlay** (logo, bell, avatar) floating over the hero.
