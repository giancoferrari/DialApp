...
- **Phase B (dark values + toggle):** not yet done.
- **Phase C (stragglers — CourseHero, paper grain, DialRing, Skeleton, Toast, native status bar):** not yet done.
- **Not yet done:** `[data-theme="dark"]` override block, `lib/theme.ts` (`getTheme`/`applyTheme`/`initTheme`, `localStorage` key `dial_theme`), Settings → Appearance section, `theme-color` meta swap.
