...
_Last updated: 2026-06-13 — Home hero **full-bleed to the top on mobile** (absolute transparent TopNav overlay + taller hero behind the floating header), plus the earlier hero **blend** (bottom mask-gradient fade into cream) + **tighter type scale** on the Dashboard (greeting/rank/stat cards) and RanksModal, matching the owner's reference. Earlier: 2026-06-11 — **Redesign v3 "Warm Clubhouse"** (Home rebuilt from the owner's prototype; warm tokens app-wide). Earlier: Redesign v2 "Private Club" (all screens), Phases 6–7 onboarding + social feel, Phases 1–5 React Query/skeletons/toasts/gestures/tokens/transitions. 
