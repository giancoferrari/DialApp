...
- **Surfaces:** frosted glass (`backdrop-filter: blur()`), rounded corners (`radius` scale), soft shadows (`elevation`).
...
### Feel / interaction polish (Phases 1–5)
...
Remaining phases (not yet built): **6** full onboarding flow, **7** social feel (comments bottom sheet, profile polish). Streaks deferred (needs data plumbing).

_Last updated: 2026-06-03 — **Phases 4–5:** design-token system (`lib/tokens.ts`), app-wide switch to the **system Helvetica** font, shared Avatar/EmptyState/format primitives, feed 4:5, directional page transitions + standardized spring-eased modal/sheet entrances (post→detail expand). Earlier: Phases 1–3 (React Query, skeletons, toasts, double-tap like, gestures, haptics); comment likes/replies, post tagging + reposts, notifications, expanded Terms/Privacy. 
