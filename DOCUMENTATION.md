...
## 5. Design system — **v2 (June 2026 redesign, in rollout)**

> ⚠️ **A full visual redesign is underway** (cool light neutrals, one typeface, flat surfaces). Tokens + Auth, TopNav, Dashboard, Feed, RecapCard, RankUpMoment, Profile, ProfilePosts, CountryPicker and Toast are on **v2**. All other screens (Scorecard, Matches, Messages, Tools/Stats/Bag/DialIn/Practice, Settings, Notifications, Friends, Leaderboard, Onboarding, LogShotModal, LegalModal, MessagesView…) still carry the old cream/green inline styles and must be migrated to the v2 language when touched. The full spec lives in the repo-root `DESIGN.md`.

- **Design tokens — `src/lib/tokens.ts` is the single source of truth.** Exports `font`, `space` (strict 4/8pt scale), `radius` (concentric: 10 controls → 12 buttons → 16 cards → 20 hero → 24 sheets), `color` (semantic names), `elevation` (flat/sm/md/lg), `z`, `motion`/`ease`, and `type` presets (whole-pixel scale 11–34). Never hardcode hexes or decimal font sizes in new code.
- **Fonts:** **Fraunces** (editorial serif — `font.display`) for the big moments only: scores, points numerals, headings, the wordmark, section headers (often italic). **Geist** (`font.body`) for ALL functional UI: buttons, labels, body, inputs, nav. Both from Google Fonts in `index.html`. Never put the serif in buttons/labels/data tables. Numbers in columns use `.tnum` / `numeric`. **No uppercase letter-spaced eyebrow labels** — section labels are sentence case 13px/600.
- **The Home hero ("Private Club" art direction):** an immersive deep-green hero (`HERO_BG` radial in `Dashboard.tsx`, base `color.greenDark #112B1D`) with faint topographic course-contour SVG linework (`ContourArt`), Fraunces italic greeting, 46px serif points numeral, DialRing around the avatar, tier-colored progress bar. On mobile it's full-bleed and **TopNav takes `tone="dark"`** (set in `App.tsx`) to blend into it; the porcelain content sheet docks over the hero with rounded top corners. On desktop the hero is a rounded block and the top bar stays light. This is the template for future hero moments on other screens.
...
### Redesign v2 (2026-06-09, in rollout — "preview first")
A ground-up visual redesign to a professional, non-"AI-looking" language: **cool light neutrals** (`#F4F5F2` bg, white flat cards, hairline borders), **one typeface (Geist)** replacing Bricolage Grotesque + DM Sans, **green `#1E4D38` as the single brand color**, glass only on the mobile bottom nav + modal scrims, sentence-case labels (uppercase eyebrows retired), whole-pixel type scale, SVG icons instead of text glyphs. **Migrated so far:** `tokens.ts`, `surfaces.ts`, `index.html`/`index.css` (fonts/bg/focus ring), `App.tsx` (bg + SetNewPasswordModal), `TopNav` (segmented desktop nav, ghost icon buttons, glass bottom bar), `AuthScreen` (card-less centered column, link-style mode switch), `Dashboard` (flat green rank hero, white stat cards, quiet leaderboard row, RanksModal), `Feed`, `RecapCard`, `RankUpMoment`, `ProfileView`, `ProfilePosts` (Composer/PostDetail/ReportSheet), `CountryPicker`, `Toast`, `Avatar`, `Button`, `Field`, `Skeleton`, `DialWordmark` (ink "Dial" + green period), `Icons` (+`RepostIcon`, `ChevronLeftIcon`). **Still on v1 (migrate when touched):** ScorecardView, MatchesView, MessagesView, ToolsView, StatsView, BagView, DialInView, PracticeView, SettingsView, NotificationsView, FriendsView, LeaderboardView, Onboarding, LogShotModal, LegalModal, EmptyState partially. Full spec: repo-root `DESIGN.md`.

_Last updated: 2026-06-09 — **Redesign v2 preview** (see above). Earlier: Phases 6–7 — first-run **onboarding** flow and **social feel** (inline feed comment preview, profile tabular numbers). Phases 1–5 — React Query, skeletons, toasts, double-tap like, gestures/haptics, design tokens (`lib/tokens.ts`), shared Avatar/EmptyState/format primitives, directional page transitions + spring modal entrances. 
