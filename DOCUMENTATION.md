...
- **Font:** **Geist** (one neutral grotesk for everything), loaded from Google Fonts in `index.html` (`wght 300..800`). `font.display` and `font.body` both point to Geist — hierarchy comes from **size + weight only** (400 body / 500 labels / 600 buttons / 650–700 headings). Numbers in columns use `.tnum` / `numeric`. **No uppercase letter-spaced eyebrow labels** — section labels are sentence case 13px/600.
