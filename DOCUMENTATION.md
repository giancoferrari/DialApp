...
- **`notifications`** — `user_id` (recipient), `type` ('post_tag' | 'repost' | 'like' | 'comment' | 'friend_accept'), `actor_id`, `post_id`, `read_at`. (`type` is free text, so new kinds need no migration.) Created by `lib/notifications.createNotification` from `posts.ts` (like/comment/repost/tag) and from `FriendsView`/`NotificationsView`'s accept handlers (`friend_accept`, 2026-07-06, `post_id: null`).
...
- **Onboarding gate (`components/Onboarding.tsx`):** once the profile has loaded (`profileLoaded`), if the signed-in user has **no `username`** (a new account), `AppShell` returns the full-screen `Onboarding` flow. **No duplicate questions:** name, username and country are already collected at sign-up, so Onboarding **prefills them from `existingProfile` → falls back to `user.user_metadata`** (set by `signUp`) and **skips the `name`/`country` steps entirely** when they're already known. It then only asks what sign-up didn't: handicap, home course, photo (all skippable). The step list is computed dynamically (`haveName`/`haveCountry`). **The handicap step is just a numeric input + "Skip for now"** (2026-07-06) — there used to be a "New to golf / Casual / Competitive" skill-level picker too, but `commit()` never read it, so it was a lie; removed rather than wired up, since skill level isn't used anywhere else in the app either. On finish it `upsertProfile(...)` (persisting name/username/country too, so the gate clears) and `setProfile(savedProfile)`. Branded green welcome/finish bookends a cream one-question-per-screen flow with a top progress bar.
...
>
> **Adoption status (2026-07-06): every screen and modal is on v4.** `PageHeader` is adopted by all list/content screens (Friends, Notifications, Messages inbox, Tools, Dial In, Practice, Settings, Stats, Leaderboard, Rounds/Scorecard history, Matches — Home/Profile/Settings keep bespoke headers by design). `useStaggerMount` is adopted by every screen, each with exactly one mount animation. `surfaces.raised` marks the one hero card per screen where used (Stats, Leaderboard, Matches wallet, Rounds summary; Profile's identity card on desktop only). All modals (`LegalModal`, `CourseSearch` dropdown, `SetNewPasswordModal`, scoring/new-match sheets, `StatsPanel`) are on the standard scrim/`radius.sheet`/`elevation.lg` recipe and render through `<Portal>`. Layout widths/padding everywhere come from `page`. Don't assume a screen is fully v4 just because it reads warm-cream; check this doc's §6 entry for specifics per screen.
...
**Monochrome + adaptive:** single-color "Dial." in Bricolage Grotesque. ⚠️ Do NOT revert to multicolour — this was changed per owner request for contrast on any background. **Defaults to `color.ink`** (theme-aware — near-black in light mode, near-white in dark mode) so it reads correctly against any themed surface. Pass an explicit `tint` prop (a fixed hex, not a token) only when it sits on something that ISN'T theme-aware — the always-light Home hero photo (`tint="#0B0D0A"`, used by `TopNav`) or an always-pine onboarding screen (`tint="#FFFFFF"`, used by `Onboarding.tsx`). **2026-07-08 fix:** the prop used to be a plain `onDark` boolean defaulting to hardcoded black, so the logo (and `TopNav`'s bell/message icons, which shared the same bug via `color.ink`) went invisible in dark mode once scrolled past the Home hero into the dark frosted header — `color.ink` used to be a safe stand-in for "the light-hero-photo-safe dark color" only because it happened to always be dark pre-dark-mode; now that it flips per theme, the hero-photo overlay state needs its own fixed color. `TopNav` computes `overHeroImage` (`isMobile && view === 'dashboard' && !scrolled`) to pick the fixed dark tint only while the transparent header sits directly on the photo; everywhere else (scrolled, any other screen, desktop) uses the theme-aware default. Same root cause fixed in `Dashboard.tsx`'s greeting heading (`color.ink` → fixed `#0B0D0A`, since it always sits over the hero photo too).
...
The five content blocks (`rankCard`, `statsGrid`, `ctaRow`, `feedBlock`, and the `greetingBlock(onPhoto)` function) are declared once in `Dashboard.tsx` and **composed differently per layout** — mobile and desktop share the exact same markup, only the arrangement differs.

**Layout order (mobile, <768px):**
1. **Illustration** — `CourseHero` fills from the very top of the screen, running behind the floating transparent header.
2. **Greeting** — absolutely positioned over the pale sky (`top: safe-area + 62px`); green date (15/600) + bold heading (33px, 700, −0.05em tracking). Called as `greetingBlock(true)` — see the `onPhoto` note below.
...
**Layout (desktop/tablet ≥768px, rebuilt 2026-07-08).** The old desktop layout was the mobile one squeezed into a 560px centred column with the greeting overlapping a near-square hero — on a wide screen it read as a stretched phone screenshot. Now:
- **`.home-shell`** (in `index.css`) caps the page: **680px** at 768–1023px, **1040px** at ≥1024px.
- **Hero row (`.home-hero-row`)** — greeting **beside** the art (never overlapping it) at ≥1024px, stacked below it under that. The art is a rounded `radius.sheet` card with `--raised-shadow`, `aspect="16 / 11"`, `fadeStart={100}` (**no** bottom fade — a crisp edge suits a framed card, unlike the mobile full-bleed blend). Desktop greeting is 44px.
- **Body grid (`.home-body-grid`)** — at ≥1024px: **feed as the main column (1.35fr) + a sticky at-a-glance sidebar (1fr, `top: 84px`)** holding rank card, stats and CTAs. Below 1024px it collapses to one column and `.home-side-col { order: -1 }` lifts those blocks **above** the feed (they'd otherwise be buried under it).
- ⚠️ **The 1024px split has to live in CSS**, not TSX: `isMobile` (`useIsMobile`) is a single 768px boolean and cannot express a second breakpoint.
- The `.hero-settle` zoom is nested **inside** the rounded frame, not on it — on the frame it would scale the whole card 6% on mount and briefly eat into the grid gap.

**Hero (`CourseHero.tsx`):**
- Loads `public/course-hero.jpg` (the coastal illustration photo); falls back to `CourseHeroArt.tsx` (the SVG version) on error.
- Props: `aspect` (default `"390 / 318"`) and `fadeStart` (default `55`) — the mobile hero uses `aspect="390 / 500"` and `fadeStart={72}` so it's taller and the fade starts later, leaving more blended illustration below the greeting; the desktop hero card uses `aspect="16 / 11"` and `fadeStart={100}` (fade disabled).
...
### Theming / dark mode (2026-07-06)
**Architecture:** every color in `lib/tokens.ts` (`color.*`, `elevation.*`, `glass.*`, `type.label.color`) is a string reference to a CSS custom property defined in `src/index.css`'s `:root` block (e.g. `color.green` → `'var(--c-green)'` → `--c-green: #12371F`), so **no component changes when a theme is added or changed** — components still write `color.green` exactly as before. Naming: `--c-<tokenKeyInKebabCase>` (`greenDeep` → `--c-green-deep`). Non-token but color-encoding values got the same treatment: `--scrim` (the standard modal-scrim `rgba(23,26,23,0.45)`, swept into ~12 files that used to hardcode it), `--shadow-sm/md/lg` (the `elevation` shadow strings), `--glass-bg/--glass-border/--glass-shadow`.
- **Phase A (done):** tokens → CSS vars, zero visual change (light-mode `:root` values are byte-identical to the old hardcoded hex). Two spots did hex+alpha string concatenation on a token color (`ScorecardView.tsx`'s scoring-summary chips: `x.color + '18'`; `Onboarding.tsx`'s pine-hero radial: `${color.greenMid}22`) — both refactored to `color-mix(in srgb, ${x.color} N%, transparent)` since `'var(--c-x)' + '18'` would've produced invalid CSS. `RankUpMoment.tsx`'s `tier.color + 'NN'` was **left untouched** — `tier.color` comes from `lib/points.ts`'s rank-tier palette, a separate fixed hex palette (Bronze/Silver/Gold/…) that was never a `tokens.color` value, so it isn't theme-dependent. Also added `color.dangerBg`/`color.dangerBorder` (`--c-danger-bg`/`--c-danger-border`) for the repeated inline "error block" recipe (`#FBEDEB` bg + `#EFCBC5` border + `color.dangerDeep` text) used across ~13 files, and `--c-label-text` for `type.label`'s previously-hardcoded `#30332F`.
- **Phase B (done) — dark values + toggle:** `index.css` gained a `:root[data-theme="dark"]` block — a warm "dark clubhouse", not pure black (cream→`#141712`, sheet→`#1B1F19`, card→`#20241E`, ink→`#F2EFE6`, pine CTA brightened to `#3C7A4B` so it still reads on dark, orange/gold/birdie/danger all brightened similarly; shadows deepen to black-based rgba; `--scrim` → `rgba(0,0,0,0.6)`). Only keys that actually needed to change got a dark entry — anything omitted (e.g. `--c-green-mid`, `--c-sage`, `--c-on-green`) keeps its light value in both themes because it already reads fine on a dark surface. New `lib/theme.ts`: `getTheme()`/`saveTheme()` (`localStorage` key `dial_theme`, values `'light'|'dark'|'system'`), `applyTheme()` (resolves `'system'` via `matchMedia('(prefers-color-scheme: dark)')`, sets `document.documentElement.dataset.theme`, updates the `theme-color` meta tag), `initTheme()` (called from `main.tsx` **before** `createRoot(...).render(...)` to avoid a flash of the wrong theme; also attaches a `matchMedia` change listener so a `'system'` user's theme follows a live OS change). **Settings → new "Appearance" section** (above About): a `Segment` control (Light/Dark/System) using the existing primitive.
  - Found two more places that needed dark-aware treatment beyond the plain token sweep: (1) the golf-notation cell tints (`ScoreDecoration`/`MatchScoreDecoration`'s eagle/birdie/bogey/double-bogey backgrounds — `#F7F0DC`/`#EAF3EC`/`#F9EEDF`/`#F9E9E6`, duplicated identically in `ScorecardView.tsx` and `MatchesView.tsx`) — promoted to `color.eagleTint`/`birdieTint`/`bogeyTint`/`doubleTint` (`--c-*-tint` vars) so they don't stay bright-pastel islands on a dark scorecard; (2) `MatchesView.tsx`'s "Won" chip, which did `rgba(176,136,40,0.22)` (a decimal-encoded `color.gold`) — refactored to `color-mix(in srgb, ${color.gold} 22%, transparent)`, same category of bug as the Phase A hex-concat fixes. (3) `TopNav.tsx`'s bottom-pill-nav and scroll-frosted-header backgrounds were hardcoded `rgba(255,250,239,…)` (never sourced from the unused `glass` export) — new `--nav-glass`/`--nav-glass-border`/`--nav-glass-shadow`/`--header-glass` vars, each with a dark counterpart.
- **Phase C (done) — stragglers:** `CourseHero`'s photo + `CourseHeroArt`'s SVG (both painted with fixed warm colors, not tokens) get a `.course-hero-media` class that dims via `filter: brightness(0.82) saturate(0.9)` under `[data-theme="dark"]`, so the Home/Profile banner doesn't blow out against the dark cream fade. `DialRing`'s default `trackColor` → `var(--ring-track)`. `Skeleton`'s shimmer gradient → `var(--skeleton-shimmer)`. `Toast` inverts (dark chip in light mode → light chip in dark mode, via `--toast-bg`/`--toast-bg-error`/`--toast-fg`/`--toast-accent`/`--toast-shadow`) so it doesn't vanish against a dark page. `native.ts` gained `setNativeTheme(theme)` (StatusBar style, guarded by `isNativePlatform`), called from `applyTheme()`. (There was no "paper grain" `feTurbulence` overlay to update — that line in this doc was stale/described a feature that isn't actually in the code; removed.)
  - **Found three more surface-level misses while doing the final sweep** (things that would've silently stayed light-colored in dark mode because they bypassed the token/var system entirely): `lib/surfaces.ts`'s `card`/`raised` recipes hardcoded `#FFFFFF`/`#FFFEFB` directly instead of `color.card`/a proper var (now `color.card` and new `--c-raised`/`--raised-border`/`--raised-shadow`, with several pine-hero-card call sites that had copy-pasted the same raised shadow literal — `MatchesView`, `ScorecardView`, `SettingsView`, `ProfileView`, `ToolsView` — updated to the var too); and **`App.tsx`'s full-screen background wash** (the fixed radial+linear cream gradient behind all content) was a hardcoded literal with no theme awareness at all — promoted to `--app-bg` with a dark counterpart. This last one would have been the most visible bug: without it, the entire app would've kept a light cream background behind every screen even with dark mode "on."
...
Full-width post cards from **you + accepted friends** (`fetchFeedPosts`). Inline like (heart, optimistic), comment count, tap media/comment → shared `PostDetail`. Tap author → their profile (`onViewProfile`). Images `loading="lazy"`. Posts are **photos**, **round recaps**, or **match recaps** (`post.kind`): recaps render via `RecapCard` instead of an `<img>`. There are two card layouts (compact caption+thumbnail, and full-width) — both share the same `Header(post)` and `Actions(post)` render helpers (2026-07-06) so avatar/name/timestamp and the like/comment/repost row look and behave identically regardless of layout; previously the compact card's repost control was icon-only with no label. `onMediaTap` takes the click event's `timeStamp` as a `now` param rather than calling `Date.now()` internally, to stay render-pure.

**"New posts" pill (2026-07-06):** Feed subscribes to `postgres_changes` INSERT on the `posts` table; an insert from anyone else sets `hasNew`, which shows a floating pine pill (rendered through `<Portal>`, fixed just under the top bar) — it does **not** auto-refetch, since a mid-scroll content jump is hostile. Tapping it invalidates `['feed', userId]`, scrolls to top (`#app-scroll-container` on mobile — the id `App.tsx` puts on the scrollable content div — or `window` on desktop), and clears the flag. RLS may limit which INSERTs are actually delivered to the subscription; that's fine, the pill just won't fire for posts this user can't see. Channel is cleaned up on unmount.

### Round recap posts — `components/RecapCard.tsx`, `lib/posts.createRoundPost`
Posts can be a **round recap** or a **match recap**: `posts.kind` ('photo' | 'round' | 'match') + `posts.meta` jsonb (`RoundRecapMeta`: course, score, par, toPar, holes, GIR, fairways, putts; or `MatchRecapMeta`: course, gameMode, holes, myScore, topOpponentName/Score, result won/lost/tied, wager, playedAt — `Post.meta` discriminates on `kind`, callers cast explicitly since `kind` isn't a true TS discriminant on the `Post` interface). After finishing a round, **ScorecardView's summary has a share block** (`createRoundPost`); after a match completes, **MatchesView's `ScoringModal` has a matching "Share this match to your feed" button** (`createMatchPost`, 2026-07-06) — the recap picks the best-scoring opponent as the "vs" line. `RecapCard` renders a green card in three variants (`feed`/`detail`/`tile`) and branches on a `kind: 'round' | 'match'` prop. Requires `SOCIAL_V5.sql` (makes `image_url` nullable, adds `kind` + `meta`) — `kind` is unconstrained `text` with no CHECK constraint, so **no further SQL migration was needed** to add the `'match'` value (confirmed by reading `SOCIAL_V5.sql` before implementing).

**Round recaps can carry a photo (2026-07-06).** `createRoundPost(userId, meta, caption?, imageUrl?)` gained the optional `imageUrl` param (`posts.image_url` was already nullable — no migration needed, confirmed against `SOCIAL_V5.sql`, so no `SOCIAL_V7.sql` either). ScorecardView's summary share block (replacing the old single button) is a `surfaces.card`: a dashed "Add photo" thumbnail (`radius.sm`, file input, live preview + remove ×) next to a `well`-style caption input, then a "Share to feed" primary button — photo uploads via the existing `uploadPostImage` (which already runs it through `lib/imageCompress` at 1440px) before the post is created. `RecapCard` gained an optional `imageUrl` prop (round-kind only): **tile** variant shows the photo as the whole tile with a small pine score-chip overlay (bottom-left, `radius.sm`, score + to-par) instead of the all-pine placeholder; **feed**/**detail** compose the photo (4∶3, object-fit cover) directly above the pine recap panel as one visual unit — that composition happens at the call site (`Feed.tsx`, `ProfilePosts.tsx`), not inside `RecapCard`, so double-tap-to-like/heart-burst (which lives on `Feed.tsx`'s existing `onMediaTap` button) still covers the photo since it's rendered inside that same button. Posts without a photo render exactly as before.
...
Mostly read-only, computed from logged rounds + shots (no schema). Shows an **estimated handicap** (best 8 of last 20 complete 18-hole rounds' to-par × 0.96), a **recent-rounds bar chart** colored by to-par, **averages** (GIR%, fairways%, putts/18, scrambling%), and **bag gap analysis** (consecutive club-distance gaps, flags "full club" gaps ≥18yd). Pure functions live in `lib/stats.ts`. **"Set as my handicap"** (2026-07-06): when an estimate exists, a quiet button under it calls `upsertProfile(userId, { handicapIndex: hcp.value })`; the button becomes a checked "Synced to profile" state once `profile.handicapIndex === hcp.value`. `StatsView` takes `profile`/`userId`/`onProfileSaved` props from `App.tsx` (mirrors the `SettingsView` save pattern) — it's no longer purely read-only.
...
Header via `PageHeader` (onBack). Sections: Account, Location (CountryPicker), Security, Golf Profile (incl. **Preferred tee**), Game Defaults (**Default holes**), Goals, **Appearance** (Light/Dark/System theme — `lib/theme.ts`, see §5 "Theming / dark mode"), About (LegalModal), Sign out, **Danger zone**. (There is no Privacy section — see §8.) The dark profile-header card keeps its pine background but takes radius/shadow from `surfaces.raised` (it's the screen's hero moment). Saves via `upsertProfile`. Avatar upload here + on ProfileView. **Danger zone → Delete account:** opens a Portal confirmation sheet requiring the user to type `DELETE`, then calls `deleteAccount()` (`lib/profile.ts` → `supabase.rpc('delete_my_account')`, see `DELETE_ACCOUNT.sql`) and signs out. This permanently erases all their data **and** their `auth.users` row.

**Game defaults are real, not decorative (2026-07-06):** `lib/prefs.ts` exports `Prefs` (`defaultHoles: 9 | 18`, `teePreference: string`) + `loadPrefs()`/`savePrefs()` (localStorage-backed, no DB). `ScorecardView.tsx` seeds its `tee`/`holeCount` state from `loadPrefs()` (both the initial `useState` and the "Add new course" reset button); `MatchesView.tsx`'s `NewMatchModal` seeds `holes` the same way. There used to be a "Distance units" row too — removed (see §8), nothing in the app ever converted yards↔meters.

### Notifications — `components/NotificationsView.tsx`, `lib/notifications.ts`
Friend requests + match invites with accept/decline + activity feed from `notifications` table (tagged, reposted, liked, commented, friend-accepted). Realtime. Opening marks read. Bell badge = pending friend requests + match invites + unread notifications. **Accepting a friend request notifies the requester** (2026-07-06): both `FriendsView.handleAccept` and `NotificationsView.handleFriendAccept` call `createNotification(requesterId, meId, 'friend_accept', null)` after `updateFriendship` succeeds (kept at the call sites rather than inside `lib/friends.updateFriendship` itself, to avoid a `friends.ts` ↔ `notifications.ts` circular import — mirrors how tag/repost notifications are created alongside the action in `posts.ts`, not inside a lower-level data module).
...
- **Files:** `supabase.ts`, `profile.ts` (incl. `deleteAccount()`), `friends.ts`, `rounds.ts`, `courses.ts`, `courseCorrections.ts`, `shots.ts`, `practice.ts`, `matches.ts`, `wallet.ts`, `points.ts`, `feed.ts`, `messages.ts`, `posts.ts`, `notifications.ts`, `golfCourseApi.ts`, `countries.ts`, `imageCompress.ts`, `prefs.ts` (localStorage-only game defaults — no DB, no `toX()` converter needed).
...
- **Anything overlaid on the Home hero photo (or any other fixed-light image/asset) must use a fixed color, never a theme-aware `color.*` token.** The hero photo doesn't change with the app theme, so text/icons drawn on top of it need to stay dark regardless of theme — `color.ink` etc. will flip light in dark mode and go invisible. See `DialWordmark`/`TopNav`/`Dashboard` greeting in §5/§6 for the pattern (`overHeroImage` check, fixed `tint`/`#0B0D0A`). **The flip side matters too:** the same element off the photo must use the tokens. `Dashboard`'s `greetingBlock(onPhoto)` takes exactly this as a parameter — mobile passes `true` (it sits on the photo → fixed `#0B0D0A` / `#3E653E`), desktop passes `false` (it sits beside the art on the themed page → `color.ink` / `color.birdie`). Hardcoding either one for both layouts breaks the other.
...
- **v4 conversion status:** every screen and modal is now on v4 (2026-07-06), including **Rounds/Scorecard**, **Matches**, `LegalModal`, `CourseSearch`, and `SetNewPasswordModal` (inside `App.tsx`). The deep-pine `onHero` tokens remain for the Messages thread header (intentional pine accent) — `HERO_BG` was removed (zero consumers). Dead illustration components (`OrganicGraphic.tsx`, `FairwayStrip.tsx`, `FlagPin.tsx`, `CourseContour.tsx`) were deleted — all unused since the v3/v4 redesign.
...
- **No "Distance units" setting (removed 2026-07-06):** the yards/meters toggle in Game Defaults never converted anything — `BagView`, `DialInView`, and `lib/format.ts`'s `formatYards` are yards-only. Removed rather than pretending; add real unit conversion as separate future work if needed.
- **No profile-privacy controls (removed 2026-07-06):** Settings previously had a Privacy section (show handicap / show rank / profile visibility) that only wrote to `localStorage` — nothing server-side enforced it, since `lib/friends.ts`'s `PROFILE_SELECT` always returns handicap/rank to any caller. Shipping fake privacy was worse than none, so it was deleted rather than kept. **Real privacy needs server-side work**: RLS policies or a view that conditionally omits fields based on a real `profiles` column, not a client-only preference. Future work, not yet started.
...
| Date | Change |
|------|--------|
| 2026-07-08 | **Home rebuilt for desktop** — the desktop Home was the mobile layout squeezed into a 560px centred column with the greeting overlapping a near-square hero, which read as a stretched phone screenshot on a wide screen (user-reported). Rebuilt as: `.home-shell` width caps (680px at 768–1023, 1040px at ≥1024), a hero row with the greeting **beside** the art (no overlap) at ≥1024px, and a body grid with the **feed as the main column + a sticky at-a-glance sidebar** (rank/stats/CTAs) — collapsing to one column below 1024px with `order: -1` lifting the sidebar above the feed. The five content blocks are now declared once and composed per layout, so mobile and desktop share identical markup. **The 1024px split lives in `index.css`** because `isMobile` is a single 768px boolean. The desktop hero art drops the bottom mask fade (crisp edge in a framed card) and nests `.hero-settle` inside the frame so the card doesn't scale into the grid gap. `greetingBlock` became `greetingBlock(onPhoto)` so the desktop copy uses theme tokens while mobile keeps the fixed on-photo colors (see §8) |
| 2026-07-08 | **Dark-mode Home-hero contrast fixes + Matches card corner bug** — the Home greeting, `TopNav`'s logo/bell/message icons, and the "Won" match card were all found broken in real dark-mode use (user-reported): the greeting heading and `TopNav`'s unscrolled-mobile icon color used the theme-aware `color.ink`, which flips to a light color in dark mode and goes invisible against the Home hero photo (a fixed-light image, never themed) — fixed with a fixed `#0B0D0A` tint plus `TopNav`'s new `overHeroImage` check (transparent header directly over the photo vs. the frosted/themed header everywhere else). `DialWordmark`'s `onDark` boolean (hardcoded black/white) is now a `tint` override defaulting to theme-aware `color.ink`, which also fixed the same invisible-logo bug on the loading screen, password-reset modal, and auth screen. Separately, `MatchesView.tsx`'s pine `MatchCard` (used for active matches and **won** completed matches) was missing `borderRadius` entirely — square corners next to every other rounded card — added `radius.lg`. |
| 2026-07-06 | **Dark mode Phase C — stragglers (dark mode complete)** — `CourseHero`/`CourseHeroArt` dim slightly in dark mode (fixed warm illustration colors, not tokens); `DialRing`, `Skeleton`, `Toast` moved their remaining hardcoded colors to vars (Toast inverts light/dark chip so it doesn't vanish); `native.ts` gained `setNativeTheme()` for the Capacitor status bar. Final sweep also caught real bugs that had bypassed the token system entirely: `lib/surfaces.ts`'s `card`/`raised` recipes were hardcoded hex (fixed, plus several call sites that had copy-pasted the raised shadow literal instead of importing it), and **`App.tsx`'s full-screen background wash was completely hardcoded with zero theme awareness** — without promoting it to `--app-bg`, the whole app would have kept a light cream background behind every screen regardless of the theme toggle. Third and last phase — dark mode is now feature-complete (`REDESIGN_PROMPTS.md` Prompt 10 done) |
| 2026-07-06 | **Dark mode Phase B — dark values + toggle** — `index.css` gains a `:root[data-theme="dark"]` override block (warm "dark clubhouse," not pure black); new `lib/theme.ts` (`getTheme`/`saveTheme`/`applyTheme`/`initTheme`, `localStorage` key `dial_theme`, resolves `'system'` via `matchMedia` + live-updates on OS change), called from `main.tsx` before render to avoid a flash of the wrong theme. Settings gains an **Appearance** section (Light/Dark/System). Also promoted the golf-notation cell tints (eagle/birdie/bogey/double, previously duplicated hex in both `ScorecardView.tsx` and `MatchesView.tsx`) to `color.*Tint` vars, fixed a decimal-encoded-hex bug in `MatchesView`'s "Won" chip (`rgba(176,136,40,…)` → `color-mix`), and gave `TopNav.tsx`'s bottom-nav/header frost (previously hardcoded, not sourced from tokens) new `--nav-glass`/`--header-glass` vars. Second phase of the dark-mode prompt — see §5 |
| 2026-07-06 | **Dark mode Phase A — tokens become CSS variables (no visual change)** — every `lib/tokens.ts` color value (`color.*`, `elevation.*`, `glass.*`, `type.label.color`) now references a `var(--c-x)` custom property defined in `index.css`'s `:root` (light values, byte-identical to the old hex); added `--scrim`, `--shadow-sm/md/lg`, `--glass-bg/border/shadow`. Fixed two hex+alpha string-concat spots that would've broken (`ScorecardView.tsx` scoring chips, `Onboarding.tsx` pine radial) via `color-mix(in srgb, …)`; added `color.dangerBg`/`dangerBorder` for the repeated inline error-block recipe (~13 files) and `--c-label-text` for `type.label`'s hardcoded hex. First phase of the dark-mode prompt (`REDESIGN_PROMPTS.md` Prompt 10) — see §5 "Theming / dark mode" |
| 2026-07-06 | **Photos on round recaps** — `createRoundPost` gained an optional `imageUrl` param (no migration needed, `posts.image_url` already nullable). ScorecardView's summary share button became a share block: optional dashed photo thumbnail (uploads via `uploadPostImage`, 1440px-compressed) + optional caption input + "Share to feed". `RecapCard` gained an `imageUrl` prop for round-kind: photo-as-tile with a pine score-chip overlay in the profile grid, photo-above-recap-card in feed/detail (composed at the call site so the existing double-tap-to-like button still covers the photo). No photo = unchanged rendering. Big-feature prompt (`REDESIGN_PROMPTS.md` Prompt 9) |
| 2026-07-06 | **"New posts" pill (Feed)** — subscribes to realtime `posts` INSERTs; an insert from someone else sets a flag that shows a floating pine pill under the top bar (no auto-refetch — a mid-scroll jump would be hostile). Tap invalidates the feed query, scrolls to top, clears the flag. `App.tsx`'s scrollable content div gained `id="app-scroll-container"` so `Feed.tsx` (nested inside `Dashboard.tsx`) can reach it without threading a ref down two levels. Fourth and last item of the small-additions prompt (`REDESIGN_PROMPTS.md` Prompt 8 complete) |
| 2026-07-06 | **Friend-accept notification** — accepting a friend request now notifies the requester (`AppNotification['type']` gains `'friend_accept'`; `notifIcon()` renders `UsersIcon` at the positive tint; `NotificationsView`'s text map adds "accepted your friend request"). Added at both `FriendsView.handleAccept` and `NotificationsView.handleFriendAccept` — no migration needed, `notifications.type` is free text. Third item of the small-additions prompt |
| 2026-07-06 | **Match recap posts** — `PostKind` gains `'match'`; new `MatchRecapMeta` type + `lib/posts.createMatchPost()`. A completed match in `MatchesView`'s `ScoringModal` shows a "Share this match to your feed" button (mirrors the Rounds summary's share button) that posts course, game mode, holes, my score, the best-scoring opponent, win/loss/tie result, and wager. `RecapCard.tsx` now takes a `kind: 'round' | 'match'` prop and renders a trophy-accented variant for matches (all three variants: feed/detail/tile). No SQL migration needed — `SOCIAL_V5.sql`'s `posts.kind` is unconstrained `text`. Second item of the small-additions prompt |
| 2026-07-06 | **"Set as my handicap" (Stats)** — the estimated-handicap hero card grows a secondary button that syncs the estimate to `profile.handicapIndex` via `upsertProfile`; shows a checked "Synced to profile" state when they already match. `StatsView` gained `profile`/`userId`/`onProfileSaved` props. First item of the small-additions prompt (`REDESIGN_PROMPTS.md` Prompt 8) |
| 2026-07-06 | **Wallet Withdraw removed** — the balance is virtual/no monetary value (Terms §8), so a "Withdraw" button implied real money movement it couldn't deliver. `MatchesView`'s wallet card is now Add-only, with a "Virtual balance · for friendly wagers" caption; `lib/wallet.ts`'s `withdrawFromWallet()` is left in place, unused, for possible future real-money support. Fourth and last item of the feature-cleanup prompt (`REDESIGN_PROMPTS.md`) |
| 2026-07-06 | **Onboarding skill-level picker removed** — the handicap step's "New to golf / Casual / Competitive" buttons saved nothing (`commit()` never read `skill`); step is now just a numeric handicap input + "Skip for now", with sub-copy "Helps us track your progress. You can skip this." Third item of the feature-cleanup prompt |
| 2026-07-06 | **Game defaults wired up for real** — new `lib/prefs.ts` (moved `Prefs`/`loadPrefs`/`savePrefs` out of `SettingsView.tsx`). `ScorecardView.tsx`'s initial `tee`/`holeCount` and the "Add new course" reset now read `loadPrefs()` instead of hardcoding `'white'`/`18`; `MatchesView.tsx`'s `NewMatchModal` seeds `holes` the same way. Removed the "Distance units" row/field entirely — nothing in the app ever converted yards↔meters, so the toggle did nothing. Second item of the feature-cleanup prompt |
| 2026-07-06 | **Settings → Privacy section removed** — the three controls (show handicap / show rank / profile visibility) only wrote to `localStorage`; nothing server-side enforced them (`lib/friends.ts` always returns handicap/rank to any viewer). Deleted the section, the `Toggle` component (now unused), and the `showHandicap`/`showRank`/`visibility` fields from `SettingsView.tsx`'s `Prefs` type + `DEFAULT_PREFS`. First item of the feature-cleanup prompt — real privacy needs RLS/view changes server-side (§8) |
| 2026-07-06 | **Stragglers → v4 + dead-code sweep** — last v2 hold-outs converted: `App.tsx`'s `SetNewPasswordModal` and `LegalModal.tsx` now use tokens throughout (standard scrim, `radius.sheet`, `elevation.lg`) and render through `<Portal>`; `CourseSearch.tsx` input + dropdown retoned (`well`/`card`/`elevation.md`). **Feed unification:** `Feed.tsx`'s compact and full-width post cards now share one `Header(post)`/`Actions(post)` render helper each, so avatar/name/timestamp and the like/comment/repost row are identical in both layouts (previously the compact card's repost control was icon-only); fixed an eslint "impure function during render" regression by threading the click event's `timeStamp` into `onMediaTap` instead of calling `Date.now()` inside it. **Dead code removed:** `OrganicGraphic.tsx`, `FairwayStrip.tsx`, `FlagPin.tsx`, `CourseContour.tsx` (zero importers), `data.ts`'s unused `makeInitialShots()` mock + duplicate `timeAgo()` (canonical version lives in `lib/format.ts`), `tokens.ts`'s unused `HERO_BG` export, a duplicate `@keyframes slideUp` in `index.css`. Corner-radius stragglers (mobile bottom sheets using `24px 24px 0 0`) bumped to the radius-law's `28px 28px 0 0` across `BagView`, `CountryPicker`, `LogShotModal`, `MessagesView`, `PracticeView`, `ProfilePosts`, `ProfileView`, `SettingsView`. Sixth prompt of the redesign pass — every screen and modal is now v4 |
...
### Older history
- **Redesign v2 "Private Club" (2026-06):** cool light-neutral content + deep-green heroes — superseded by v3/v4 tokens; the `onHero` tokens remain for non-Home screens (`HERO_BG` and the v2-era illustration components were removed 2026-07-06, see changelog).
